...
export default function RolesPage() {
  ...
  const adminRoles = useMemo(
    () =>
      roles
        .filter((r) => r.appliesTo === "Admin")
        .filter((r) => showCustom || !isCustomRoleCode(r.code)),
    [roles, showCustom],
  );
  ...
  function typeBadge(role: RoleDto) {
    ...
    if (isCustomRoleCode(role.code)) return <Badge variant="outline">{t("type.custom")}</Badge>;
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-4 pb-3">
          <p className="text-xs text-muted-foreground">
            {isLoading ? tCommon("loading") : tCommon("resultsFound", { count: adminRoles.length })}
          </p>
