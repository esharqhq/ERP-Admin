...
interface Props {
  ...
  pending: boolean;
  /**
   * The one sentence that is about *this* subject, supplied by the caller from
   * its own namespace — an owner and a worker read the resulting ticket in
   * different apps, and the copy says so. Everything else on the form is the
   * same form.
   */
  title: string;
  description: string;
...
/**
 * Opens a support ticket addressed to a subject — an owner or a worker.
 *
 * Deliberately not a chat: their app renders this as a ticket, so the exchange
 * keeps a status and a full history rather than scrolling away, and an admin
 * instruction can be pointed at later.
 *
...
export function MessageUserDialog({
  ...
  const t = useTranslations("detail.message");
  ...
  const priorityItems = TICKET_PRIORITIES.map((p) => ({
    value: p,
    label: t(`priorities.${p}` as Parameters<typeof t>[0]),
  }));
    ...
    <Dialog open={open} onOpenChange={(v) => !v && !pending && onClose()}>
      ...
      <DialogContent className="max-h-[calc(100dvh-4rem)] overflow-y-auto">
        ...
        <div className="flex flex-col gap-1.5">
          <Label id="ticket-category-label">{t("category")}</Label>
          <div
            role="group"
        ...
        <div className="flex flex-col gap-1.5">
          <Label id="ticket-priority-label">{t("priority")}</Label>
          <div
            role="group"
        ...
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="ticket-body">{t("body")}</Label>
          <textarea
            id="ticket-body"
            value={body}
